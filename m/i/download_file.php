<?php
require_once('../be_assets/ss_includes/config.php');
require_once('../be_assets/ss_includes/check_login.php');
...
if ( (abs($getf)>0) && (strlen($geti)==10) )
	...
	$sql_perm="SELECT value_path,perm_read,sysprofiles_idsysprofiles,wfassetsdata.wfprocassets_idwfprocassets as assets,
	(SELECT perm_read FROM wfprocassetsaccess WHERE wfprocassets_idwfprocassets=assets AND sysprofiles_idsysprofiles=".$_SESSION['WASReBSyS_iduserprofile'].") as perm
	 FROM wfassetsdata 
	...
	if ($num_perm<1) //then there is no record so give warning and exit
		...
		VALUES ('".$fet_perm['value_path']."', '0', '0', '0', '".$timenowis."', '".$_SESSION['WASReBSyS_idacname']."', '".session_id()."', '".$_SERVER["REMOTE_ADDR"]."', 'NOLK')";				
		...
		if ($fet_perm['perm']!=1)
			...
			//log that failure
			$sql_download="INSERT INTO audit_docdownlds ( doc_name, doc_ext, doc_size, tktin_idtktin, createdon, createdby, usersess, usrip, usrresult) 
			VALUES ('".$fet_perm['value_path']."', '0', '0', '0', '".$timenowis."', '".$_SESSION['WASReBSyS_idacname']."', '".session_id()."', '".$_SERVER["REMOTE_ADDR"]."', 'NOPM')";
			...
			if (file_exists('../documents/task_docs/'.$geti.'/'.$fet_perm['value_path'])!=1)
				...
				//log that failure
				$sql_download="INSERT INTO audit_docdownlds ( doc_name, doc_ext, doc_size, tktin_idtktin, createdon, createdby, usersess, usrip, usrresult) 
				VALUES ('".$fet_perm['value_path']."', '0', '0', '0', '".$timenowis."', '".$_SESSION['WASReBSyS_idacname']."', '".session_id()."', '".$_SERVER["REMOTE_ADDR"]."', 'NFIL')";
...
//				$fname = basename($_GET['f']);
		...
		*/		
				...
				// log this doenload
				$sql_download="INSERT INTO audit_docdownlds ( doc_name, doc_ext, doc_size, tktin_idtktin, createdon, createdby, usersess, usrip, usrresult) 
				VALUES ('".$fname."', '0', '0', '0', '".$timenowis."', '".$_SESSION['WASReBSyS_idacname']."', '".session_id()."', '".$_SERVER["REMOTE_ADDR"]."', 'OK')";
				
				mysql_query($sql_download);
							
				
				} //if (($fet_perm['perm_read']!=1) || ($fet_perm['sysprofiles_idsysprofiles']!=$_SESSION['WASReBSyS_iduserprofile']) )
