								...
								if ($fet_nextwf['wfsymbol_idwfsymbol']==10) //if it is the end of the process
									...
									$next_step="last_step";
										...
										if ($fet_actors['usrrole_idusrrole'] >0 ) //if more than 0, then it is a allocated to a role
											...
											WHERE wfactors.wftskflow_idwftskflow=".$fet_nextwf['idwftskflow']." AND usrac.usrteam_idusrteam=".$_SESSION['WkAtToMrPa_idacteam']." ".$limit_to_zone_qry1." ".$limit_to_dpt_qry1." AND acstatus=1 AND ".$_SESSION['NoRTS']." ORDER BY usrteamzone.idusrteamzone";
										...
										if ($fet_actors['usrgroup_idusrgroup'] > 0 ) //if allocated to a group, then do the following
											...
											INNER JOIN usrteamzone ON usrrole.usrteamzone_idusrteamzone=usrteamzone.idusrteamzone
											WHERE wftasks.createdon>='".$sevendaysago."' AND wftasks.createdon<='".$timenowis."' ".$limit_to_zone_qry1." ".$limit_to_dpt_qry1." AND usrac.usrteam_idusrteam=".$_SESSION['WkAtToMrPa_idacteam']." AND link_userrole_usergroup.usrgroup_idusrgroup=".$fet_actors['usrgroup_idusrgroup']." AND acstatus=1 AND ".$_SESSION['NoRTS']." GROUP BY wftasks.usrac_idusrac ORDER BY minutes ASC";
											//echo "test";
											...
											INNER JOIN usrac ON link_userrole_usergroup.usrrole_idusrrole = usrac.usrrole_idusrrole
											INNER JOIN usrteamzone ON usrrole.usrteamzone_idusrteamzone=usrteamzone.idusrteamzone
											WHERE wftasks.createdon<='".$timenowis."' AND link_userrole_usergroup.usrgroup_idusrgroup=".$fet_actors['usrgroup_idusrgroup']." AND usrac.usrteam_idusrteam=".$_SESSION['WkAtToMrPa_idacteam']." ".$limit_to_zone_qry1." ".$limit_to_dpt_qry1." AND acstatus=1 AND ".$_SESSION['NoRTS']." GROUP BY wftasks.usrac_idusrac ORDER BY minutes ASC";
											//echo "test";
