...
WHERE link_tskcategory_wfproc.usrteam_idusrteam=".$_SESSION['WkAtToMrPa_idacteam']." AND link_tskcategory_wfproc.tktcategory_idtktcategory=".$_SESSION['tktcat']." AND wfproc.wfstatus=1 LIMIT 1";
								...
								if ($num_preactors > 0)
									{
										if ($fet_preactors['usrrole_idusrrole'] > 0)
											...
											$sql_thisusr="SELECT usrrole_idusrrole FROM wfactors WHERE usrrole_idusrrole=".$_SESSION['WkAtToMrPa_iduserrole']." AND wftskflow_idwftskflow=".$fet_preactors['wftskflow_idwftskflow']." LIMIT 1";
										...
										if ($fet_preactors['usrgroup_idusrgroup'] > 0)
											{
											$sql_thisusr="SELECT usrrole_idusrrole FROM link_userrole_usergroup WHERE usrrole_idusrrole=".$fet_preactors['usrgroup_idusrgroup']." AND usrteam_idusrteam=".$_SESSION['WkAtToMrPa_idacteam']." LIMIT 1";
											$res_thisusr=mysql_query($sql_thisusr);
											$fet_thisusr=mysql_fetch_array($res_thisusr);
											
												if ($fet_thisusr['usrrole_idusrrole']==$_SESSION['WkAtToMrPa_iduserrole'])
													{
													$skip_to_step2=1; //set variable to skip to next workflow
													}
							...
							if ($fet_nextwf['wfsymbol_idwfsymbol']==10)//if it is the end of the process
								...
								$next_step="last_step";
									...
									if ($fet_actors['usrrole_idusrrole'] >0 ) //if more than 0, then it is a allocated to a role
										...
										WHERE wfactors.wftskflow_idwftskflow=".$fet_nextwf['idwftskflow']." AND usrac.usrteam_idusrteam=".$_SESSION['WkAtToMrPa_idacteam']." AND acstatus=1 AND wfactors.usrteamzone_idusrteamzone=".$regionId." ";
									...
									if ($fet_actors['usrgroup_idusrgroup'] > 0 ) //if allocated to a group, then do the following
										...
										WHERE wftasks.createdon>='".$sevendaysago."' AND wftasks.createdon<='".$timenowis."' AND usrac.usrteam_idusrteam=".$_SESSION['WkAtToMrPa_idacteam']." AND link_userrole_usergroup.usrgroup_idusrgroup=".$fet_actors['usrgroup_idusrgroup']." AND acstatus=1 AND usrrole.usrteamzone_idusrteamzone=".$regionId." GROUP BY wftasks.usrac_idusrac ORDER BY minutes ASC";
										//echo "test";
										...
										INNER JOIN link_userrole_usergroup ON usrrole.idusrrole = link_userrole_usergroup.usrrole_idusrrole
										INNER JOIN usrac ON link_userrole_usergroup.usrrole_idusrrole = usrac.usrrole_idusrrole
										WHERE wftasks.createdon<='".$timenowis."' AND link_userrole_usergroup.usrgroup_idusrgroup=".$fet_actors['usrgroup_idusrgroup']." AND usrac.usrteam_idusrteam=".$_SESSION['WkAtToMrPa_idacteam']." AND acstatus=1 AND usrrole.usrteamzone_idusrteamzone=".$regionId." GROUP BY wftasks.usrac_idusrac ORDER BY minutes ASC";
										//echo "test";
